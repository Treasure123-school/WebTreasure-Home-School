...
import { useAuth } from "@/hooks/useAuth"; // Use useAuth hook for logout logic
...
  // Correctly get initials from the full_name property
...
  // Correctly get nav items based on the user's role_name
...
  // Use the user object and correct property for navigation
  const navItems = getNavItems(user?.role_name);

  // If user is null (not authenticated), don't render the header
  if (!user) {
    return null;
  }
...
                <a 
                  onClick={() => logout()} // Use onClick for logout function call
                  className="block text-red-600 hover:text-red-700 font-medium cursor-pointer" // Add cursor-pointer
