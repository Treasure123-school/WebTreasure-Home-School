...
export default function StudentDashboard() {
  ...
  const { data: announcements = [] } = useQuery({
    queryKey: ['/api/announcements', { audience: 'students' }],
    enabled: !!user && user.role === 'student',
    retry: false,
  });
  ...
  const availableExams = exams.filter((exam: any) => 
    !submissions.some((sub: any) => sub.examId === exam.id)
  );
                    ...
                    <div key={submission.id} className="flex items-center justify-between p-4 bg-backgroundSurface rounded-lg">
                      <div className="flex-1">
                        <div className="font-medium text-textPrimary">
                          {/* We would need exam title here - in real implementation, this would be joined data */}
                          Exam Result
                        </div>
        ...
        <Card data-testid="student-announcements">
          ...
          <CardContent>
            {announcements.length === 0 ? (
              <p className="text-textSecondary text-center py-4">No announcements available</p>
            ) : (
              <div className="space-y-4">
